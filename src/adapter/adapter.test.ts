...
describe(`MetaStorage`, () => {
	...
	describe.each([
		...
		['mysql', getConnection, createMysqlMetaStorage],
	])('Adapters', (adapterName, getConnnection, createMetaStorage) => {
		
		describe(`${adapterName}`, () => {
			beforeAll(async function() {
				dbConnection = await getConnnection(adapterName);
				metaStorage = await createMetaStorage(dbConnection);
			});

			afterAll(async function() {
				await metaStorage.close()
			});
